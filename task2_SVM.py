...
def get_train_x_y():
    ...
    # x = manifold.TSNE(n_components=2, init="random", random_state=501).fit_transform(X)
...
cv_cross = cross_validate(svm_model, x_train, y_train, cv=kfold,
                          scoring=("accuracy", "f1"))
print(cv_cross)

# fpr, tpr and auc (plot)
y_train_pre = svm_model.predict_proba(x_train)
y_train_0 = list(y_train_pre[:, 1])
fpr, tpr, thresholds = roc_curve(y_train, y_train_0)
train_auc = roc_auc_score(y_train, y_train_0)
print("Train auc: ", train_auc)

y_pre = svm_model.predict_proba(x_test)
y_0 = list(y_pre[:, 1])
fpr, tpr, thresholds = roc_curve(y_test, y_0)
test_auc = roc_auc_score(y_test, y_0)
