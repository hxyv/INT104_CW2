...
for (name, method, colorname) in zip(names, methods, colors):
    ...
    cv_cross = cross_validate(svm_model, x_train, y_train, cv=kfold, scoring=("accuracy", "f1"))
    print(cv_cross)

    # fpr, tpr and auc (plot)
    y_test_predprob = svm_model.predict_proba(x_test)[:,1]
